...
function CheckoutContent() {
  ...
  useEffect(() => {
    const cashfreeOrder = searchParams.get("cashfreeOrderId");
    if (!cashfreeOrder) return;
    fetch(`/api/cashfree/status?orderId=${encodeURIComponent(cashfreeOrder)}`).then(r => r.json()).then(d => { if (d.status === "SUCCESS") { setPaymentMethod("CASHFREE"); setPaymentDone(true); toast.success("Payment confirmed"); } }).catch(() => {});
  }, [searchParams]);
  ...
  const upiLink = `upi://pay?pa=6302094687@fam&pn=Varahi Eat & Fit&am=${grandTotal}`;
