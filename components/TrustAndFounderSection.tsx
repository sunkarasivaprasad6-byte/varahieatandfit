...
const OWNER_IMAGE = "/images/owner.jpeg";
const FSSAI_NUMBER = "20126260001032";
const FSSAI_CERTIFICATE = "";
    ...
    <section className="bg-[#050607] px-5 py-20 text-white sm:px-8 lg:px-10">
      <div className="mx-auto grid max-w-[1450px] gap-8 lg:grid-cols-[1.05fr_0.95fr]">
        <div className="rounded-[28px] border border-green-400/20 bg-green-400/[0.04] p-7 sm:p-10">
          ...
          <p className="mt-4 max-w-2xl text-white/55">Varahi Eat & Fit is proud to display its food-safety certification.</p>
          <div className="mt-7 flex flex-wrap items-center gap-4">
            ...
            <div>
              <p className="text-sm font-bold">Food Safety & Standards Authority of India</p>
              <p className="mt-1 text-sm text-white/40">Registration No. {FSSAI_NUMBER}</p>
              {FSSAI_CERTIFICATE && <a href={FSSAI_CERTIFICATE} target="_blank" rel="noreferrer" className="mt-2 inline-block text-xs font-bold text-green-300 hover:underline">View certificate</a>}
            </div>
          </div>
        </div>
        ...
        <div className="relative overflow-hidden rounded-[28px] border border-white/10 bg-[#101112] p-7 sm:p-10">
          ...
          <div className="relative flex flex-col items-center gap-7 sm:flex-row sm:items-center">
            <div className="relative h-48 w-40 shrink-0 overflow-hidden rounded-3xl border border-white/10 bg-white/5">
              <Image src={OWNER_IMAGE} alt="Varahi Eat & Fit owner" fill className="object-cover" sizes="160px" priority />
            </div>
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.3em] text-[#E63946]">Meet the Owner</p>
              <h2 className="mt-3 text-3xl font-black sm:text-4xl">The person behind Varahi Eat & Fit</h2>
              <p className="mt-4 text-sm leading-7 text-white/50">We are proud to introduce the owner of Varahi Eat & Fit.</p>
            </div>
          </div>
        </div>
      </div>
