...
export default function PaymentSection({ paymentMethod, setPaymentMethod, paymentDone, setPaymentDone, upiTransactionId, setUpiTransactionId, grandTotal, upiLink }: Props) {
  ...
  function openUPI() { let appOpened = false; const handleVisibility = () => { if (document.hidden) appOpened = true; }; document.addEventListener("visibilitychange", handleVisibility); window.location.href = upiLink; setTimeout(() => { document.removeEventListener("visibilitychange", handleVisibility); if (!appOpened) toast.error("No UPI app found. Scan the QR code or choose Cash on Delivery."); }, 2000); }
  ...
  function submitPayment() { setTransactionTouched(true); if (!normalizedTransactionId) { toast.error("Please enter your UPI Transaction ID / UTR"); return; } if (!transactionIdValid) { toast.error("Please enter a valid UPI Transaction ID / UTR"); return; } setUpiTransactionId(normalizedTransactionId); setPaymentDone(true); }
