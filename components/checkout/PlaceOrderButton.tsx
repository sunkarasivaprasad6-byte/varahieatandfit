...
export default function PlaceOrderButton({
  ...
  // Prevent duplicate automatic UPI submissions.
  ...
  const handlePlaceOrder = async () => {
    ...
    if (placingOrder || orderGenerated || submissionStarted.current) {
      return;
    }
...
Thank you for ordering from *Varahi Eat & Fit* ❤️`;
  ...
  }, [paymentDone, isCOD, orderGenerated, placingOrder]);
    ...
    <div className="w-full">
      ...
      {isCOD && (
        <div className="mb-5 rounded-2xl border border-blue-500/20 bg-blue-500/10 p-5">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="text-blue-400 font-bold">Cash on Delivery</h3>
      ...
      {isCOD && !orderGenerated && (
        <button
          ...
          className="w-full bg-[#E63946] hover:bg-red-600 disabled:bg-[#7f242c] disabled:cursor-not-allowed rounded-2xl py-4 text-lg font-bold text-white transition-all duration-300 hover:scale-[1.01] active:scale-[0.98] shadow-lg flex items-center justify-center gap-3"
        >
          {placingOrder ? (
            <>
